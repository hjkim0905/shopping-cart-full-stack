...
orderRouter.post('/preview', (req: Request, res: Response) => {
  ...
  if (!Array.isArray(coupons) || coupons.length > MAX_COUPONS) {
  ...
  // 후보 쿠폰: 명시된 쿠폰만 후보로 사용한다. (빈 배열이면 쿠폰 미적용)
  const candidates = DB.Coupons.filter((coupon) => coupons.includes(coupon.id));
